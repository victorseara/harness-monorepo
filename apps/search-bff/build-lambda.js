...
async function buildLambda() {
  try {
    const result = await esbuild.build({
      ...
      bundle: true,
      outfile: '../../dist/apps/search-bff/main.js',
      platform: 'node',
      target: 'node22',
      format: 'cjs',
    ...
    console.log('Output:', path.resolve('../../dist/apps/search-bff/main.js'));

    // Check file size
    const fs = require('fs');
    const stats = fs.statSync('../../dist/apps/search-bff/main.js');
    console.log(`Bundle size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);

    // Save build analysis
    if (result.metafile) {
      fs.writeFileSync(
        '../../dist/apps/search-bff/meta.json',
        JSON.stringify(result.metafile, null, 2)
      );
      console.log(
        'Build analysis saved to ../../dist/apps/search-bff/meta.json'
      );
